...
function showDictionaryPopup(results) {
    ...
    // Audio (Play first char or all? Let's play the first one to be subtle)
...
// --- LONG PRESS OVERLOAD LOGIC ---

function getTargetButton(e) {
    // Look for specific game buttons. We traverse up just in case clicked on an inner span.
...
function handleStart(e) {
    ...
    // Prepare lookup
    // Note: We use textContent. Clean up whitespace.
    ...
    longPressTimer = setTimeout(() => {
        ...
        if (results.length > 0) {
            // SUCCESS: Show popup and block the upcoming click
...
}, { capture: true }); // Capture phase is key

// Block Context Menu
document.addEventListener('contextmenu', function(e) {
    if (getTargetButton(e)) {
        // Only block context menu if we actually have a long-pressable button
        e.preventDefault();
        return false;
    }
});
