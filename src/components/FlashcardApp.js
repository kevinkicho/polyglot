...
export class FlashcardApp {
    ...
    toggleFlip() { 
        ...
        // Auto-play sentence on flip to back
        const settings = settingsService.get();
        const item = this.currentData;
        if(this.isFlipped && settings.autoPlay && item && item.back.sentenceTarget) {
            setTimeout(() => {
                 // Using regex to strip HTML if sentence contains bold tags etc
                 const clean = item.back.sentenceTarget.replace(/<[^>]*>?/gm, '');
                 audioService.speak(clean, settings.targetLang);
            }, 300);
        }
    }
