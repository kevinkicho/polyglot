...
export class BlanksApp {
    ...
    async handleAnswer(id, el) { 
        ...
        if(correct) {
            ...
            if(settings.blanksAutoPlayCorrect) {
                const promise = this.playAudio(true);
                if(settings.blanksWaitAudio) await promise;
            }
            setTimeout(() => this.next(), settings.blanksWaitAudio ? 100 : 1000);
        }
    }
    ...
    playAudio(full = false) { 
        ...
        const settings = settingsService.get();
        if (full) return audioService.speak(this.currentData.cleanSentence, settings.targetLang);
        else {
            const parts = this.currentData.sentence.split('_______');
            if (parts.length > 1) return audioService.speakGapSentence(parts[0], parts[1], settings.targetLang);
            else return audioService.speak(this.currentData.sentence, settings.targetLang);
        }
    }
        ...
        this.container.innerHTML = `
            <div class="fixed top-0 left-0 right-0 h-16 z-40 px-4 flex justify-between items-center bg-gray-100/90 dark:bg-dark-bg/90 backdrop-blur-sm">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button id="blanks-random-btn" class="header-btn bg-white dark:bg-dark-card border border-gray-200 rounded-xl text-indigo-500 shadow-sm"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg></button>
                    <button id="blanks-close-btn" class="header-btn bg-red-50 dark:bg-red-900/20 text-red-500 shadow-sm"><svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                </div>
            </div>
