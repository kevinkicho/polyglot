...
export class BlanksApp {
    ...
    random() { this.isAnswered=false; audioService.stop(); this.currentData=blanksService.generateQuestion(null); this.render(); }
    next(id=null) {
        this.isAnswered=false; audioService.stop();
        if(id) this.currentData = blanksService.generateQuestion(id);
        else {
            const l=vocabService.getAll(); const i=vocabService.findIndexById(this.currentData.target.id);
            this.currentData = blanksService.generateQuestion(l[(i+1)%l.length].id);
        }
    ...
    handleAnswer(id, el) { 
        ...
        if(correct) {
            document.getElementById('blank-slot').classList.replace('bg-gray-200', 'text-green-600');
            document.getElementById('blank-text').classList.remove('invisible');
            if(settingsService.get().blanksAutoPlayCorrect) audioService.speak(this.currentData.cleanSentence, settingsService.get().targetLang);
            setTimeout(() => this.next(), 1000);
        }
    }

    render() {
        if(!this.container) return; if(!this.currentData) { this.container.innerHTML="No Data"; return; }
        const { target, sentence, answerWord, choices } = this.currentData;
        const fontClass = settingsService.get().targetLang === 'ja' ? 'font-jp' : '';
        ...
        if(settingsService.get().targetLang==='ja' && parts[0].includes('、')) rendered = rendered.replace(/、/g, '、<br>');
        ...
        this.container.innerHTML = `
            <div class="fixed top-0 left-0 right-0 h-16 z-40 px-4 flex justify-between items-center bg-gray-100/90 dark:bg-dark-bg/90 backdrop-blur-sm">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button id="blanks-close-btn" class="w-10 h-10 bg-red-50 text-red-500 rounded-full flex items-center justify-center shadow-sm"><svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </div>
            </div>
        ...
        document.getElementById('blanks-id-input').addEventListener('change', (e) => this.next(parseInt(e.target.value)));
