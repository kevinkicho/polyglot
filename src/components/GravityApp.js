...
export class GravityApp extends BaseGameComponent {
    ...
    spawnAsteroid() {
        ...
        el.innerHTML = `
            ...
            <span class="text-xs font-bold text-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full text-center px-1 pointer-events-none drop-shadow-md leading-tight">${this.textService.smartWrap(item.front.main)}</span>
        `;
    ...
    updateTargetDisplay(slotIdx) {
        ...
        if (el && target) {
            ...
            this.textService.fitText(el, 14, 20);
        }
    }
