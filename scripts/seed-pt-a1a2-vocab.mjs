/**
 * Seed script: uploads Portuguese A1 & A2 vocabulary data to Firebase Realtime Database.
 *
 * Usage:
 *   node scripts/seed-pt-a1a2-vocab.mjs
 *
 * Prerequisites:
 *   - Node.js 18+
 ...
 *   4. Run: node scripts/seed-pt-a1a2-vocab.mjs
 *
 * The script will:
 ...
 *
 * IDs start at 60000 to avoid collisions with existing data.
 */
...
try {
  ...
  console.error('Or set the SERVICE_ACCOUNT_KEY environment variable to the key file path.');
  process.exit(1);
}
...
// ── Data ──────────────────────────────────────────────────────────
const { ptA1A2VocabData } = await import('./pt-a1a2-vocab-data.mjs');

// ── Seed Logic ────────────────────────────────────────────────────
async function seed() {
  console.log(`Seeding ${ptA1A2VocabData.length} Portuguese A1/A2 vocabulary entries...\n`);
  ...
  for (const entry of ptA1A2VocabData) {
    ...
    if (added % 10 === 0) {
      console.log(`  Prepared ${added} / ${ptA1A2VocabData.length} entries...`);
    }
  }
  ...
  if (added > 0) {
    ...
    console.log(`\nAll ${ptA1A2VocabData.length} entries already exist. Nothing to seed.`);
  }
