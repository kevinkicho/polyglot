/**
 * Seed script: uploads B2 vocabulary data to Firebase Realtime Database.
 *
 * Usage:
 *   node scripts/seed-b2-vocab.mjs
 *
 * Prerequisites:
 *   - Node.js 18+
 ...
 *   4. Run: node scripts/seed-b2-vocab.mjs
 *
 * The script will:
 ...
 *
 * IDs start at 70000 to avoid collisions with existing data.
 */
...
// ── Data ──────────────────────────────────────────────────────────
const { b2VocabData } = await import('./b2-vocab-data.mjs');

// ── Seed Logic ────────────────────────────────────────────────────
async function seed() {
  console.log(`Seeding ${b2VocabData.length} B2 vocabulary entries...\n`);
  ...
  for (const entry of b2VocabData) {
    ...
    if (added % 10 === 0) {
      console.log(`  Prepared ${added} / ${b2VocabData.length} entries...`);
    }
  }
  ...
  if (added > 0) {
    ...
    console.log(`\nAll ${b2VocabData.length} entries already exist. Nothing to seed.`);
  }
